    ...
    <div v-if="!snapchatError" class="m-4 d-flex flex-column">
      ...
      <b-card-group class="flex justify-content-center flex-row">
        ...
        >
          <b-card
            bg-variant="light"
            :header="`Account Name: ` + snapchatAdAccount.adaccount.name"
            class="m-2 fb-card"
          >
            <b-card-text>
              <p class="fb-text">
                Currency: {{ snapchatAdAccount.adaccount.currency }}
              </p>
              <p class="fb-text">
              ...
              <b-button
                ...
                :class="{
                  disabled: alreadyAdded(snapchatAdAccount.adaccount.id),
                }"
                :disabled="alreadyAdded(snapchatAdAccount.adaccount.id)"
                variant="primary"
                @click="handleClick(snapchatAdAccount.adaccount)"
                >{{
...
export default {
  ...
  methods: {
    async handleClick(adaccount) {
      try {
        const result = await axios.post("/snapchatadaccount", adaccount);
